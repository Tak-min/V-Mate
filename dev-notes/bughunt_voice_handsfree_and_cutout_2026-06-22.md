...
> **【2026-06-22 追記・訂正】** 本ログの「不具合1」で挙げた `requiresOnDeviceRecognition`
> フォールバックは、シミュレータビルドだけで立てた仮説であり**真因ではなかった**。実機ログを
> 取って判明した真の原因は **VADしきい値のスケール不一致**(iOSの実RMSがWebスケールの
> しきい値に届かず発話未検出)。詳細と確定証拠は
> [`bughunt_ios_vad_threshold_scale_2026-06-22.md`](./bughunt_ios_vad_threshold_scale_2026-06-22.md)
> を参照。なお「不具合2(音声の途切れ=TTS先読み)」の修正は有効なまま。
