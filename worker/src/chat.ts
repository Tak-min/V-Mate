...
import { EMOTION_TAG_RE, stripEmotion, stripTags, sse, jstToday, timeContext } from "./util";
...
export function handleChat(
  ...
  const work = (async () => {
    ...
    let fullText = "";
    try {
      for await (const chunk of streamChat(env, system, history)) {
        buffer += chunk;
        if (emotion === null) {
    ...
    if (fullText) await store.addMessage(uid, "assistant", fullText, emotion);
