...
export function introNamePrompt(transcript: string): string {
  const emotions = EMOTIONS.join("|");
  return `あなたは「シロ」。ユーザーと今まさに初めて出会い、音声で「お名前、聞いてもいい？」と
尋ねたところ、ユーザーが声でこう答えた(音声認識のため誤字・言い淀みを含むことがある):

「${transcript}」

この発言から、ユーザーが名乗った・呼んでほしいと言った名前(呼び名・ニックネーム可)だけを
抽出し、以下の形式で**必ず2行**で出力せよ。

1行目: 抽出した名前だけ(敬称・「です」「だよ」等の語尾は含めない)。名前が聞き取れない・
　　　　言っていない・雑談で名前ではないと判断した場合は「なし」とだけ書く。
2行目: [${emotions}] のいずれか1つで始まる、その名前を優しく肯定的に受け止める1〜2文の返事。
　　　　例: [happy] ○○ちゃんかあ、すてきな名前だね。これからよろしくね。
　　　　1行目が「なし」のときは、聞き取れなかったことを気にさせない優しい一言(もう一度
　　　　聞いてもいい、というニュアンス)にする。
　　　　「ユーザー」「あなた」ではなく、抽出できた名前があればその名前で語りかける。

出力は2行のみ。前置き・説明・引用符は付けない。`;
}
